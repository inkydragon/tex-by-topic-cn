...
在处理文件时，\TeX\ 总处于某种模式(\indexterm{mode/模式})。
一共有六种模式，可划分为三类：
\begin{enumerate}
\item 水平模式和受限水平模式，
\item 竖直模式和内部竖直模式，
...
%horizontal mode.
水平模式(\indextermbus{horizontal/水平}{mode/模式})的主要用来构建一行行的文字。
...
水平列表(\indextermbus{horizontal/水平}{list/列表})的每一个元素都是以下各项之一：
\begin{itemize}
...
竖直列表(\indextermbus{vertical/竖直}{list/列表})的每一个元素都是以下各项之一：
\begin{itemize}
